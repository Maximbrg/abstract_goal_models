...
for i in range(len(mandatory)):
    ...
    EL = Ta + Qu + AB + CO + Con
    Dec = (((len(me_map_graph.me_map_graph.nodes) - len(graph.me_map_graph.nodes))) / (
        len(me_map_graph.me_map_graph.nodes)))
...
for node in nodes_labels:
    if nodes_labels[node][1] == 'Task':
        Ta += 1
    if nodes_labels[node][0] == 'Quality':
        Qu += 1

for edge in edge_labels:
    if edge_labels[edge] == 'AchievedBy':
        AB += 1
    if edge_labels[edge] == 'ConsistsOf':
        CO += 1
    else:
        Con += 1

EL = Ta + Qu + AB + CO + Con
Dec = (((len(me_map_graph.me_map_graph.nodes) - len(nodes_labels))) / (
    len(nodes_labels)))
# Dec = (len(graph.me_map_graph.nodes) / (len(me_map_graph.me_map_graph.nodes))) * 100
print("|new map| TA %f | Qu %f | AB %d | CO %d | Con %d | El %d| Dec %f | Abst.(msec) %f" %
      (Ta, Qu, AB, CO, Con, EL, Dec, (end - start) * 1000))
